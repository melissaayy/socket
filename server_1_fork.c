...
int main(int argc, char const* argv[]) 
{
	...
	while(1) // main accept() loop
	{
        ...
        if ((childpid = fork()) == 0)
        {
            // Closing the server socket id cuz the child do not need the listening socket 
            close(sockfd);

            // Send a confirmation message
            // to the client
            send(new_fd, "hi client",
                 strlen("hi client"), 0);
            ...
            while(1)
            {
                ...
                if (strcmp(strToRecv, "close") == 0) {
                    printf("server: received close command, connection with requested client closed!!!\n");
                    break; 
                }
            }
